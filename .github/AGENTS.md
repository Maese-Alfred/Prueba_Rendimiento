# AGENTS.md — ASDD Project (k6 Performance Testing)

> Canonical shared version: this file is the source of truth for shared agent guidelines.

This file defines general guidance for all AI agents working in this repository, following the **ASDD (Agent Spec Software Development)** workflow adapted for **k6 performance testing**.

## Project Summary

> Ver `README.md` en la raíz del proyecto para stack, convenciones y estructura de carpetas.
> Ver `.github/README.md` para la estructura completa del framework ASDD.

## ASDD Workflow

**Every new performance test must follow this pipeline:**

```
[FASE 1 — Secuencial]
spec-generator    → /generate-spec      → .github/specs/<test-name>.spec.md

[FASE 2 — Secuencial]
k6-developer      → scripts/ (script principal, config, helpers, checks, data)
...
| implement-k6 | `/implement-k6` | Implementa script k6 completo y modular |

### QA
| Skill | Slash Command | Descripción |
|-------|---------------|-------------|
| performance-analyzer | `/performance-analyzer` | Planifica y analiza pruebas (Load/Stress/Spike/Soak) |
| gherkin-case-generator | `/gherkin-case-generator` | Genera escenarios Given-When-Then de performance |
| risk-identifier | `/risk-identifier` | Clasifica riesgos con Regla ASD (Alto/Medio/Bajo) |
| automation-flow-proposer | `/automation-flow-proposer` | Propone flujos a automatizar y hoja de ruta |
...
|---|---|---|
| Lineamientos de Desarrollo k6 | `.github/docs/lineamientos/dev-guidelines.md` | k6 Developer |
| Lineamientos QA | `.github/docs/lineamientos/qa-guidelines.md` | QA Agent |
| Reglas de Oro | `.github/AGENTS.md` | Todos (siempre activas) |
| Stack k6 + convenciones | `.github/instructions/k6.instructions.md` | k6 Developer, Spec Generator |
| Diccionario de dominio | `.github/copilot-instructions.md` | Todos |
...
- **Preservar la lógica existente**: respetar patrones de modularización, convenciones de naming y thresholds definidos en la spec.
...
|------|-----------|-------------|
| Requerimientos de performance | `.github/requirements/` | Input: descripción del test (endpoint, SLAs, tipo de prueba) |
| Especificaciones técnicas | `.github/specs/` | Output del Spec Generator, fuente de verdad para el k6 Developer |

## Critical Rules for All Agents

- Sin spec `APPROVED` → no hay scripts — sin excepciones.
- Nunca hardcodear URLs base ni credenciales — siempre variables de entorno (`__ENV`).
- Nunca incluir datos reales de producción en CSV o scripts.
- Scripts de performance siempre con `sleep()` — no atacar servicios sin think time.
- Thresholds obligatorios en toda spec: p(95) y error rate como mínimo.
